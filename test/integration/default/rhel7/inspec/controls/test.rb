...
control "xccdf_org.cisecurity.benchmarks_rule_2.1.1_Ensure_chargen_services_are_not_enabled" do
  ...
  describe files.delete_if { |f| command("file #{f} | cut -d: -f2").stdout =~ /binary|executable|archive/ || file(f).content =~ /^chargen +/ } do
    it { should be_empty }
  end
end
...
control "xccdf_org.cisecurity.benchmarks_rule_2.1.2_Ensure_daytime_services_are_not_enabled" do
  ...
  files = command("find /etc/inetd.d -type f -regex .\\*/.\\+").stdout.split
  describe files.delete_if { |f| command("file #{f} | cut -d: -f2").stdout =~ /binary|executable|archive/ || file(f).content =~ /^daytime +/ } do
    it { should be_empty }
  end
end
...
control "xccdf_org.cisecurity.benchmarks_rule_2.1.3_Ensure_discard_services_are_not_enabled" do
  ...
  files = command("find /etc/inetd.d -type f -regex .\\*/.\\+").stdout.split
  describe files.delete_if { |f| command("file #{f} | cut -d: -f2").stdout =~ /binary|executable|archive/ || file(f).content =~ /^discard +/ } do
    it { should be_empty }
  end
end
...
control "xccdf_org.cisecurity.benchmarks_rule_2.1.4_Ensure_echo_services_are_not_enabled" do
  ...
  files = command("find /etc/inetd.d -type f -regex .\\*/.\\+").stdout.split
  describe files.delete_if { |f| command("file #{f} | cut -d: -f2").stdout =~ /binary|executable|archive/ || file(f).content =~ /^echo +/ } do
    it { should be_empty }
  end
end
...
control "xccdf_org.cisecurity.benchmarks_rule_2.1.5_Ensure_time_services_are_not_enabled" do
  ...
  files = command("find /etc/inetd.d -type f -regex .\\*/.\\+").stdout.split
  describe files.delete_if { |f| command("file #{f} | cut -d: -f2").stdout =~ /binary|executable|archive/ || file(f).content =~ /^time +/ } do
    it { should be_empty }
  end
end
...
control "xccdf_org.cisecurity.benchmarks_rule_2.1.6_Ensure_tftp_server_is_not_enabled" do
  ...
  files = command("find /etc/inetd.d -type f -regex .\\*/.\\+").stdout.split
  describe files.delete_if { |f| command("file #{f} | cut -d: -f2").stdout =~ /binary|executable|archive/ || file(f).content =~ /^tftp +/ } do
    it { should be_empty }
  end
end
...
control "xccdf_org.cisecurity.benchmarks_rule_6.2.7_Ensure_all_users_home_directories_exist" do
  ...
  passwd.where { user =~ /^(?!root|halt|sync|shutdown).*/ }.where { ! nologin.include?(shell) }.homes.each do |homefolder|
...
control "xccdf_org.cisecurity.benchmarks_rule_6.2.10_Ensure_users_dot_files_are_not_group_or_world_writable" do
  ...
  passwd.where { user =~ /^(?!root|halt|sync|shutdown).*/ }.where { ! nologin.include?(shell) }.users.each_with_index { |k,i| homeslist[k] = passwd.where { user =~ /^(?!root|halt|sync|shutdown).*/ }.where { ! nologin.include?(shell) }.homes[i] }
  homeslist.each do |user,homefolder|
    dotfiles = command('find ' + homefolder + ' -name \".[A-Za-z0-9]*\" -maxdepth 1 -not -type d -not -type l').stdout.split
    if !dotfiles.empty?
      ...
      describe "List of dotfiles for user #{user}" do
...
control "xccdf_org.cisecurity.benchmarks_rule_6.2.11_Ensure_no_users_have_.forward_files" do
  ...
  homeslist = {}
  nologin = [ '/sbin/nologin', '/usr/bin/nologin', '/bin/nologin', '/bin/false' ]
  passwd.where { user =~ /^(?!root|halt|sync|shutdown).*/ }.where { ! nologin.include?(shell) }.users.each_with_index { |k,i| homeslist[k] = passwd.where { ! nologin.include?(shell) }.homes[i] }
  homeslist.each do |user,homefolder|
